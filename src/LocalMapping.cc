...
namespace ORB_SLAM2 {
    ...
    void LocalMapping::AddPointSurface() {
        vector<KeyFrame*> vpLocalKeyFrames = mpCurrentKeyFrame->GetVectorCovisibleKeyFrames();
        set<MapPoint *> vpSurfaceMapPoints;
        for (auto pKF : vpLocalKeyFrames){
            ...
            for (int iMP = 0, iendMP = num; iMP < iendMP; iMP++) {
                ...
                if (pMP->mbInSurface)
                    vpSurfaceMapPoints.insert(pMP);
            }

            for (auto pMP : vpSurfaceMapPoints){
                mpSurface->AddSurfacePoint(pMP);
            }
            mpMap->mbUsePlane = mpSurface->FitPlaneFromMapPoints(mpCurrentKeyFrame);
            mpMap->mpSurface = mpSurface;
        }
    }
